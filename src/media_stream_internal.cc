...
rtp_error_t uvgrtp::media_stream_internal::init_connection()
{
...
rtp_error_t uvgrtp::media_stream_internal::create_media(rtp_format_t fmt)
{
    switch (fmt) {
...
rtp_error_t uvgrtp::media_stream_internal::free_resources(rtp_error_t ret)
{
    ...
    {
        std::lock_guard<std::mutex> guard(rtcp_map_mutex_);
        auto it = rtcp_map_.find(ssrc_.get()->load());
        if (it != rtcp_map_.end())
        {
            auto shared = it->second;
            unsigned use_count = (unsigned)shared.use_count();
            UVG_LOG_DEBUG("media_stream_internal::free_resources rtcp for ssrc=%u rtcp_ptr=%p use_count=%u", ssrc_.get()->load(), shared.get(), use_count);
            if (use_count <= 1) {
                UVG_LOG_DEBUG("media_stream_internal::free_resources erasing rtcp for ssrc=%u rtcp_ptr=%p", ssrc_.get()->load(), shared.get());
                rtcp_map_.erase(it);
            }
            else {
                UVG_LOG_DEBUG("media_stream_internal::free_resources keeping rtcp_map_ entry for ssrc=%u (other owners remain)", ssrc_.get()->load());
            }
        }
    }
...
rtp_error_t uvgrtp::media_stream_internal::install_packet_handlers()
{
    reception_flow_->install_handler(
        1, remote_ssrc_,
...
rtp_error_t uvgrtp::media_stream_internal::init(std::shared_ptr<uvgrtp::zrtp> zrtp)
{
    zrtp_ = zrtp;
...

    {
        ...
        if (rtcp_map_.find(ssrc_.get()->load()) == rtcp_map_.end()) {
            ...
            UVG_LOG_DEBUG("media_stream_internal::init created new rtcp for ssrc=%u rtcp_ptr=%p use_count=%u", ssrc_.get()->load(), rtcp_.get(), (unsigned)rtcp_.use_count());
        }
        else
        {
            rtcp_ = rtcp_map_[ssrc_.get()->load()];
            UVG_LOG_DEBUG("media_stream_internal::init reusing rtcp for ssrc=%u rtcp_ptr=%p use_count=%u", ssrc_.get()->load(), rtcp_.get(), (unsigned)rtcp_.use_count());
        }
    }
...
rtp_error_t uvgrtp::media_stream_internal::init_auto_zrtp(std::shared_ptr<uvgrtp::zrtp> zrtp)
{
    zrtp_ = zrtp;
...
rtp_error_t uvgrtp::media_stream_internal::start_zrtp()
{
    if (!zrtp_) {
...
rtp_error_t uvgrtp::media_stream_internal::add_srtp_ctx(uint8_t* key, uint8_t* salt)
{
    if (!key || !salt)
...
rtp_error_t uvgrtp::media_stream_internal::start_components()
{
...
rtp_error_t uvgrtp::media_stream_internal::push_frame(std::unique_ptr<uint8_t[]> data, size_t data_len, int rtp_flags)
{
...
rtp_error_t uvgrtp::media_stream_internal::push_frame(uint8_t* data, size_t data_len, uint32_t ts, int rtp_flags)
{
    rtp_error_t ret = check_push_preconditions(rtp_flags, false);
    if (ret == RTP_OK)
    {
...
rtp_error_t uvgrtp::media_stream_internal::push_frame(uint8_t* data, size_t data_len, uint32_t ts, uint64_t ntp_ts, int rtp_flags)
{
    rtp_error_t ret = check_push_preconditions(rtp_flags, false);
    if (ret == RTP_OK)
    {
...
rtp_error_t uvgrtp::media_stream_internal::push_frame(std::unique_ptr<uint8_t[]> data, size_t data_len, uint32_t ts, int rtp_flags)
{
    rtp_error_t ret = check_push_preconditions(rtp_flags, true);
    if (ret == RTP_OK)
    {
...
rtp_error_t uvgrtp::media_stream_internal::push_frame(std::unique_ptr<uint8_t[]> data, size_t data_len, uint32_t ts, uint64_t ntp_ts, int rtp_flags)
{
    rtp_error_t ret = check_push_preconditions(rtp_flags, true);
    if (ret == RTP_OK)
    {
...
/* Disabled for now
rtp_error_t uvgrtp::media_stream_internal::push_user_packet(uint8_t* data, uint32_t len)
{
    if (rce_flags_ & RCE_RECEIVE_ONLY) {
...
rtp_error_t uvgrtp::media_stream_internal::install_user_receive_hook(void* arg, void (*hook)(void*, uint8_t* payload, uint32_t len))
{
    if (!initialized_) {
...
uvgrtp::frame::rtp_frame* uvgrtp::media_stream_internal::pull_frame()
{
...
uvgrtp::frame::rtp_frame* uvgrtp::media_stream_internal::pull_frame(size_t timeout_ms)
{
    if (!check_pull_preconditions()) {
        return nullptr;
    }
...
bool uvgrtp::media_stream_internal::check_pull_preconditions()
{
    if (!initialized_) {
        UVG_LOG_ERROR("RTP context has not been initialized fully, cannot continue!");
...
rtp_error_t uvgrtp::media_stream_internal::check_push_preconditions(int rtp_flags, bool smart_pointer)
{
    if (!initialized_) {
        UVG_LOG_ERROR("RTP context has not been initialized fully, cannot continue!");
        return RTP_NOT_INITIALIZED;
...
uint8_t* uvgrtp::media_stream_internal::copy_frame(uint8_t* original, size_t data_len)
{
...
rtp_error_t uvgrtp::media_stream_internal::install_receive_hook(void* arg, void (*hook)(void*, uvgrtp::frame::rtp_frame*))
{
    if (!initialized_) {
        UVG_LOG_ERROR("RTP context has not been initialized fully, cannot continue!");
        return RTP_NOT_INITIALIZED;
...
rtp_error_t uvgrtp::media_stream_internal::configure_ctx(int rcc_flag, ssize_t value)
{
    rtp_error_t ret = RTP_OK;
    if (!initialized_) {
        UVG_LOG_ERROR("RTP context has not been initialized fully, cannot continue!");
...
int uvgrtp::media_stream_internal::get_configuration_value(int rcc_flag)
{
    int ret = -1;
...
uint32_t uvgrtp::media_stream_internal::get_ssrc() const
{
...
rtp_error_t uvgrtp::media_stream_internal::init_srtp_with_zrtp(int rce_flags, int type, std::shared_ptr<uvgrtp::base_srtp> srtp,
...
uint32_t uvgrtp::media_stream_internal::get_default_bandwidth_kbps(rtp_format_t fmt)
{
    int bandwidth = 50;
    switch (fmt) {
    case RTP_FORMAT_PCMU:
...
void uvgrtp::media_stream_internal::holepuncher()
{
    if (rce_flags_ & RCE_HOLEPUNCH_KEEPALIVE)
    {
        if (holepuncher_)
