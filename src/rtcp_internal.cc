...
uvgrtp::rtcp_internal::rtcp_internal(std::shared_ptr<uvgrtp::rtp> rtp, std::shared_ptr<std::atomic_uint> ssrc, std::shared_ptr<std::atomic<uint32_t>> remote_ssrc,
    ...
    rtcp_internal(rtp, ssrc, remote_ssrc, cname, sfp, rce_flags)
{
    srtcp_ = srtcp;
}

uvgrtp::rtcp_internal::~rtcp_internal()
{
    if (active_)
    {
        stop();
...
void uvgrtp::rtcp_internal::cleanup_participants()
{
...
void uvgrtp::rtcp_internal::free_participant(std::unique_ptr<rtcp_participant> participant)
{
...
rtp_error_t uvgrtp::rtcp_internal::stop()
{
    ...
    uvgrtp::rtcp_internal::send_bye_packet({ *ssrc_.get() });
...
rtp_error_t uvgrtp::rtcp_internal::remove_all_hooks()
{
    sr_mutex_.lock();
    sender_hook_ = nullptr;
    sr_hook_f_ = nullptr;
...
rtp_error_t uvgrtp::rtcp_internal::remove_send_app_hook(std::string app_name)
{
...
rtp_error_t uvgrtp::rtcp_internal::install_sender_hook(void (*hook)(uvgrtp::frame::rtcp_sender_report*))
{
    if (!hook)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_sender_hook(std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_sender_report>)> sr_handler)
{
    if (!sr_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_sender_hook(std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_sender_report>)> sr_handler)
{
    if (!sr_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_receiver_hook(void (*hook)(uvgrtp::frame::rtcp_receiver_report*))
{
    if (!hook)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_receiver_hook(std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_receiver_report>)> rr_handler)
{
    if (!rr_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_receiver_hook(std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_receiver_report>)> rr_handler)
{
    if (!rr_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_sdes_hook(void (*hook)(uvgrtp::frame::rtcp_sdes_packet*))
{
    if (!hook)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_sdes_hook(std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_sdes_packet>)> sdes_handler)
{
    if (!sdes_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_sdes_hook(std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_sdes_packet>)> sdes_handler)
{
    if (!sdes_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_app_hook(void (*hook)(uvgrtp::frame::rtcp_app_packet*))
{
    if (!hook)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_app_hook(std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_app_packet>)> app_handler)
{
    if (!app_handler)
    {
        return RTP_INVALID_VALUE;
...
rtp_error_t uvgrtp::rtcp_internal::install_send_app_hook(std::string app_name, std::function<std::unique_ptr<uint8_t[]>(uint8_t& subtype, uint32_t& payload_len)> app_sending_func)
{
...
rtp_error_t uvgrtp::rtcp_internal::install_app_hook(std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_app_packet>)> app_handler)
{
    if (!app_handler)
    {
        return RTP_INVALID_VALUE;
...
uvgrtp::frame::rtcp_sender_report* uvgrtp::rtcp_internal::get_sender_packet(uint32_t ssrc)
{
...
uvgrtp::frame::rtcp_receiver_report* uvgrtp::rtcp_internal::get_receiver_packet(uint32_t ssrc)
{
    std::lock_guard<std::mutex> prtcp_lock(participants_mutex_);
    if (participants_.find(ssrc) == participants_.end())
    {
...
uvgrtp::frame::rtcp_sdes_packet* uvgrtp::rtcp_internal::get_sdes_packet(uint32_t ssrc)
{
    std::lock_guard<std::mutex> prtcp_lock(participants_mutex_);
    if (participants_.find(ssrc) == participants_.end())
    {
...
uvgrtp::frame::rtcp_app_packet* uvgrtp::rtcp_internal::get_app_packet(uint32_t ssrc)
{
    std::lock_guard<std::mutex> prtcp_lock(participants_mutex_);
    if (participants_.find(ssrc) == participants_.end())
    {
...
void uvgrtp::rtcp_internal::zero_stats(uvgrtp::sender_statistics* stats)
{
    stats->sent_pkts = 0;
    stats->sent_bytes = 0;
...
void uvgrtp::rtcp_internal::zero_stats(uvgrtp::receiver_statistics* stats)
{
...
void uvgrtp::rtcp_internal::set_ts_info(uint64_t clock_start, uint32_t clock_rate, uint32_t rtp_ts_start)
{
...
void uvgrtp::rtcp_internal::update_session_statistics(const uvgrtp::frame::rtp_frame* frame)
{
    if (!frame)
        return;
...
rtp_error_t uvgrtp::rtcp_internal::recv_packet_handler_common(void* arg, int rce_flags, uint8_t* read_ptr, size_t size, frame::rtp_frame** out)
{
    (void)rce_flags;
    (void)size;
    (void)read_ptr;
...
rtp_error_t uvgrtp::rtcp_internal::send_packet_handler_vec(void* arg, uvgrtp::buf_vec& buffers)
{
    ...
    return ((uvgrtp::rtcp_internal*)arg)->update_sender_stats(pkt_size);
}

size_t uvgrtp::rtcp_internal::rtcp_length_in_bytes(uint16_t length)
{
...
rtp_error_t uvgrtp::rtcp_internal::handle_incoming_packet(void* args, int rce_flags, uint8_t* buffer, size_t size, frame::rtp_frame** out)
{
    (void)args;
    (void)rce_flags;
    (void)out;
...
rtp_error_t uvgrtp::rtcp_internal::handle_receiver_report_packet(uint8_t* buffer, size_t& read_ptr, size_t packet_end,
    uvgrtp::frame::rtcp_header& header)
{
...
rtp_error_t uvgrtp::rtcp_internal::handle_sender_report_packet(uint8_t* buffer, size_t& read_ptr, size_t packet_end,
    uvgrtp::frame::rtcp_header& header)
{
...
rtp_error_t uvgrtp::rtcp_internal::handle_sdes_packet(uint8_t* packet, size_t& read_ptr, size_t packet_end,
...
rtp_error_t uvgrtp::rtcp_internal::handle_bye_packet(uint8_t* packet, size_t& read_ptr,
    uvgrtp::frame::rtcp_header& header)
{
    (void)header;
...
rtp_error_t uvgrtp::rtcp_internal::handle_app_packet(uint8_t* packet, size_t& read_ptr,
...
rtp_error_t uvgrtp::rtcp_internal::handle_fb_packet(uint8_t* packet, size_t& read_ptr,
    size_t packet_end, uvgrtp::frame::rtcp_header& header)
{
    (void)packet;
    (void)read_ptr;
    ...
    UVG_LOG_WARN("RTCP feedback message received, but has not yet been implemented");
    return RTP_OK;
...
rtp_error_t uvgrtp::rtcp_internal::send_rtcp_packet_to_participants(uint8_t* frame, uint32_t frame_size, bool encrypt)
{
    if (!frame)
    {
...
uint32_t uvgrtp::rtcp_internal::size_of_ready_app_packets() const
{
    uint32_t app_size = 0;
...
uint32_t uvgrtp::rtcp_internal::size_of_apps_from_hook(std::vector<std::shared_ptr<rtcp_app_packet>> packets) const
{
    uint32_t app_size = 0;
...
uint32_t uvgrtp::rtcp_internal::size_of_compound_packet(uint16_t reports,
...
rtp_error_t uvgrtp::rtcp_internal::send_app_packet(const char* name, uint8_t subtype,
...
uint32_t uvgrtp::rtcp_internal::get_rtcp_interval_ms() const
{
    return interval_ms_.load();
}

rtp_error_t uvgrtp::rtcp_internal::set_network_addresses(std::string local_addr, std::string remote_addr,
...
std::shared_ptr<uvgrtp::socket> uvgrtp::rtcp_internal::get_socket() const {
    return rtcp_socket_;
}


void uvgrtp::rtcp_internal::set_session_bandwidth(uint32_t kbps)
{
    if (kbps <= 0) {
...
void uvgrtp::rtcp_internal::set_payload_size(size_t mtu_size)
{
    mtu_size_ = mtu_size;
}

void uvgrtp::rtcp_internal::set_socket(std::shared_ptr<uvgrtp::socket> socket)
{
    rtcp_socket_ = socket;
}
